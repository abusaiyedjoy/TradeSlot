  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-4xl">
      ...
      <div className="p-4 rounded-2xl bg-orange-500/10 border border-orange-500/20 text-xs text-orange-300 flex items-start gap-3">
        ...
        <div>
          <strong className="text-orange-200">MVP Scope Architecture:</strong> This is a simple, one-off daily setting. Customers requesting slots on unconfigured dates are prompted to choose an active date. (Recurring weekly zone schedules are supported in future scope).
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl space-y-6">
        <h3 className="text-lg font-bold text-white font-outfit flex items-center gap-2">
          <MapPin className="w-5 h-5 text-orange-400" /> Set Work Area for Date
        </h3>
        ...
        <form onSubmit={handleSave} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <Label htmlFor="workDate" className="text-xs font-semibold text-slate-300">
                Target Date
              </Label>
            ...
            <div className="sm:col-span-2">
              <Label htmlFor="areaLabel" className="text-xs font-semibold text-slate-300">
                Area / Zone Label
              </Label>
              <div className="mt-1.5 relative">
                <Input
            ...
            <div className="flex flex-wrap gap-2">
              {PRESET_AREAS.map((preset) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg text-xs font-medium bg-slate-950 border border-slate-800 text-slate-300 hover:text-white hover:border-slate-700 hover:bg-slate-800/60 transition-all text-left"
                >
                  {preset}
                </button>
              ))}
          ...
          <Button
            ...
            className="bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-semibold text-xs py-2.5 px-6 rounded-xl shadow-lg shadow-orange-500/20 transition-all gap-2"
          >
            {loading ? (
      ...
      <div className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl space-y-4">
        <h3 className="text-base font-bold text-white font-outfit flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400 font-normal">Available for booking</span>
        </h3>
          ...
          {savedAreas.map((area) => (
            <div key={area.id} className="py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-white">{area.areaLabel}</h4>
              ...
              <div className="flex items-center gap-2 self-end sm:self-center">
                <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[11px] font-medium">
                  Active
                </Badge>
                <Button
                  size="sm"
